*修改艺术的标题、作者和描述。

修改标题：

<img width="382" alt="标题" src="https://github.com/user-attachments/assets/b59d4aea-4996-4968-a873-80e56b324038">

修改作者：

<img width="383" alt="作者" src="https://github.com/user-attachments/assets/1f29ff9e-4a84-492f-bccb-3e3362312384">

修改描述：

<img width="387" alt="描述" src="https://github.com/user-attachments/assets/d17636cb-33d0-435b-8238-f74ca13c8543">

结果展示：

<img width="735" alt="结果" src="https://github.com/user-attachments/assets/3cb68825-570e-40fc-8f2f-81cbcd33f1dd">

太长了可以鼠标滑轮往下滚动或者拖动右边的滑动条往下拉：

<img width="735" alt="下拉" src="https://github.com/user-attachments/assets/88de40ac-b03a-48f4-a0fe-1468e804ab8e">


*能够在mod设置里设置艺术标签页的大小。

<img width="563" alt="设置" src="https://github.com/user-attachments/assets/f3ad6b7f-985a-48e3-a5c6-94d49eba452b">


*修改描述存档后会保存到存档文件里。如果想要将描述还原成原版，开启上帝模式，艺术组件多了个重置所有描述的按钮，能将描述全部还原成原版，保存后存档里修改后的描述就不见了。

<img width="398" alt="0重置描述" src="https://github.com/user-attachments/assets/d5925c39-a424-4ffb-b7a7-9625664bd024" />


*原版的描述支持Unity的颜色代码，颜色代码是一种用 RGBA 格式表示颜色的十六进制代码。使用颜色代码能改变描述的颜色。如下示例：
红色: <color=#FF0000FF>红色文本</color>

绿色: <color=#00FF00FF>绿色文本</color>

蓝色: <color=#0000FFFF>蓝色文本</color>

黄色: <color=#FFFF00FF>黄色文本</color>

青色 (Cyan): <color=#00FFFFFF>青色文本</color>



*由于代码为描述的生成方法添加 Harmony 补丁，其它修改艺术描述的模组可能无法修改描述。


*游戏没有入正，无法上传到创意工坊，库文件只能在GitHub上下载，我已仿照mod文件结构制作了完整的mod文件夹，下载解压后将里面名称为“艺术编辑器”的文件夹复制到RimWorld\Mods下，打开游戏后启用该mod即可。
